        ...
        public void Calculate(Transform handTarget) {
            if (handTarget != null) {
                ...
                if (handMovements != null && handMovements.stretchlessTarget != null) {
                    ...
                    // 0.02F is to get the arm IK stable at full stretch
                    if (distance > length - 0.02F) {
                        handMovements.stretchlessTarget.position = upperArm.position + armDirection.normalized * (length - 0.02F);
                    } else {
